	...
	public final ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
		...
		if (optionalElement.isPresent()) {
			...
			return findRepeatableAnnotations(annotatedElement, this.annotationType).stream()
					.map(annotation -> {
						ConditionEvaluationResult result = evaluate(annotation, annotatedElement);
						logResult(annotation, annotatedElement, result);
						return result;
					})
					.filter(ConditionEvaluationResult::isDisabled)
					.findFirst()
					.orElse(getNoConditionsEncounteredResult());
			// @formatter:on
		}
		return getNoConditionsEncounteredResult();
	}

	protected abstract ConditionEvaluationResult getNoConditionsEncounteredResult();

	protected abstract ConditionEvaluationResult evaluate(A annotation, AnnotatedElement annotatedElement);
